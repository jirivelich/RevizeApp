...
export function FormBuilderPage() {
  ...
  const [repeatCounts, setRC] = useState<RepeatCounts>({});
  ...
  const enterFill = () => {
    const fd: FillData = {};
    const rc: RepeatCounts = {};
    doc.pages.forEach(pg => pg.blocks.forEach(bl => {
      if (bl.kind === 'section') {
        if (bl.repeatable) {
          // Repeatable: klíče ve formátu `${id}_0` pro první instanci
          rc[bl.id] = 1;
          bl.items.forEach(item => {
            if (item.itemKind === 'field') fd[`${item.id}_0`] = item.defaultValue;
            else item.rows.forEach(r => r.cells.forEach(c => { fd[`${c.id}_0`] = ''; }));
          });
        } else {
          bl.items.forEach(item => {
    ...
    <div style={T.app}>
      ...
      {createPortal(
        <div id="PRINTROOT" style={{ display: 'none' }}>
          {doc.pages.map((pg, pi) => (
            <PrintPage key={pg.id} pg={pg} doc={doc} data={formData} repeatCounts={repeatCounts} isLast={pi === doc.pages.length - 1} />
          ))}
        </div>,
        document.body
      )}
      ...
      {mode === 'fill' && (
        ...
        >
          ...
          <div>
            {doc.pages.map((pg, pi) => (
              <FillPage key={pg.id} pg={pg} doc={doc} data={formData} setData={setFD} repeatCounts={repeatCounts} setRepeatCounts={setRC} isLast={pi === doc.pages.length - 1} />
            ))}
          </div>
        </div>
      )}
