    ...
    <div ref={ref} className="relative inline-flex items-center gap-1">
      {value.trim() && (
        <button
          ...
          className="text-xs text-[var(--success-text)] hover:opacity-80 hover:bg-[var(--success-bg)] px-1.5 py-0.5 rounded transition-colors cursor-pointer"
      ...
      <button
        ...
        className="text-xs text-[var(--text-muted)] hover:text-[var(--text)] hover:bg-[var(--bg-hover)] px-1.5 py-0.5 rounded transition-colors cursor-pointer"
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 bg-[var(--surface)] border border-[var(--border-medium)] rounded-lg shadow-xl min-w-[340px] max-w-[440px] py-1 max-h-96 overflow-y-auto">
            ...
            <>
              ...
              {builtIn.map((t, i) => (
                <button
                  key={`b-${i}`}
                  type="button"
                  className="w-full text-left px-3 py-2 hover:bg-[var(--bg-accent)] transition-colors cursor-pointer border-b border-[var(--border-subtle)]"
                  onClick={() => applyText(t.text)}
                >
                  <div className="text-xs font-semibold text-[var(--primary)]">{t.label}</div>
          ...
          {custom.length > 0 && (
            <>
              <div className="px-3 py-1.5 text-[10px] uppercase tracking-wider text-[var(--success-text)] font-semibold border-b border-[var(--border-subtle)] mt-1">Vlastní předvolby</div>
              {custom.map((t) => (
                <div key={`c-${t.id}`} className="flex items-start group">
                  <button
                    type="button"
                    className="flex-1 text-left px-3 py-2 hover:bg-[var(--success-bg)] transition-colors cursor-pointer border-b border-[var(--border-subtle)]"
                    onClick={() => applyText(t.text)}
                  >
                    <div className="text-xs font-semibold text-[var(--success-text)]">{t.nazev}</div>
                    <div className="text-xs text-[var(--text-muted)] mt-0.5 line-clamp-2">{t.text}</div>
                  </button>
                  <button
                    type="button"
                    className="px-2 py-2 text-xs text-[var(--danger-text)] opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
          ...
          <div className="border-t border-[var(--border)] mt-1">
            {!adding ? (
              <button
                type="button"
                className="w-full text-left px-3 py-2 text-xs text-[var(--primary)] hover:bg-[var(--bg-accent)] cursor-pointer font-medium"
